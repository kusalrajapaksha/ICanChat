...
- (void)audio_PCMtoMP3:(NSTimeInterval)recordTime
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(endConvertWithData:seconds:localPath:)]){
        NSData *voiceData = [NSData dataWithContentsOfFile:mp3FilePath];
        [_delegate endConvertWithData:voiceData seconds:recordTime localPath:mp3FilePath];
        [self deleteCafCache];
    }
}
...
- (NSString *)mp3Path {
   NSString * path=  MessageAudioCache([WebSocketManager sharedManager].currentChatID);
  [DZFileManager creatDirectoryWithPath:path];
    return path;
}
