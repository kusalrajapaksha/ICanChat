...
- Author: Created  by DZL on 14/8/2020
- File name:  UITextView+YBAttributeTextTapAction.m
- Description:
- Function List:
*/
        

#import "UITextView+YBAttributeTextTapAction.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>
@interface YBTextViewAttributeModel : NSObject
...
@implementation YBTextViewAttributeModel

@end
@implementation UITextView (YBAttributeTextTapAction)
...
- (id<YBTextViewAttributeTapActionDelegate>)yBdelegate
{
    return objc_getAssociatedObject(self, _cmd);
}
- (void)setYBdelegate:(id<YBTextViewAttributeTapActionDelegate>)yBdelegate
{
    objc_setAssociatedObject(self, @selector(yBdelegate), yBdelegate, OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)yb_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <YBTextViewAttributeTapActionDelegate> )delegate
{
    [self yb_removeAttributeTapActions];
    [self yb_getRangesWithStrings:strings];
    self.userInteractionEnabled = YES;
    
    if (self.yBdelegate != delegate) {
        self.yBdelegate = delegate;
    }
}
...
- (void)yb_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<YBTextViewAttributeTapActionDelegate>)delegate
{
    [self yb_removeAttributeTapActions];
    [self yb_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    BOOL ret = [self yb_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.yBdelegate && [weakSelf.yBdelegate respondsToSelector:@selector(yb_tapAttributeInLabel:string:range:index:)]) {
            [weakSelf.yBdelegate yb_tapAttributeInLabel:weakSelf string:string range:range index:index];
        }
    }];
    if (!ret) {
        [super touchesEnded:touches withEvent:event];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                YBTextViewAttributeModel *model = self.attributeStrings[j];
...
- (void)yb_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            YBTextViewAttributeModel *model = [YBTextViewAttributeModel new];
            model.range = range;
            model.str = obj;
