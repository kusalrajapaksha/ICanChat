...
#pragma mark - Global Color
    ...
    QMUICMI.grayDarkenColor = UIColor252730Color;                          // UIColorGrayDarken : 深一点的灰色
    QMUICMI.grayLightenColor = UIColorGray7;                                    // UIColorGrayLighten : 浅一点的灰色
    QMUICMI.redColor = UIColorMake(253, 94, 58);                                // UIColorRed : 红色
    ...
    QMUICMI.backgroundColor = UIColorViewBgColor;                                     // UIColorForBackground : 界面背景色，默认用于 QMUICommonViewController.view 的背景色
    ...
    QMUICMI.separatorColor = UIColorSeparatorColor;                          // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUI_Border) 分隔线颜色
    QMUICMI.separatorDashedColor = UIColorMake(17, 17, 17);                     // UIColorSeparatorDashed : 全局默认的虚线分隔线的颜色，默认 QMUIKit 暂时没用到
    QMUICMI.placeholderColor = UIColorSeparatorColor;                                    // UIColorPlaceholder，全局的输入框的 placeholder 颜色，默认用于 QMUITextField、QMUITextView，不影响系统 UIKit 的输入框
...
#pragma mark - UIButton
    ...
    QMUICMI.buttonTintColor = UIColorThemeMainColor;                              // ButtonTintColor : QMUIButton 默认的 tintColor，不影响系统的 UIButton
...
#pragma mark - TextField & TextView
    QMUICMI.textFieldTintColor = UIColorThemeMainColor;                          // TextFieldTintColor : QMUITextField、QMUITextView 的 tintColor，不影响 UIKit 的输入框
...
#pragma mark - UISwitch
    QMUICMI.switchOnTintColor = UIColorThemeMainColor;                           // SwitchOnTintColor : UISwitch 打开时的背景色（除了圆点外的其他颜色）
...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarShadowImage = [UIImage qmui_imageWithColor:UIColorBg243Color size:CGSizeMake(4, PixelOne) cornerRadius:0];                                            // NavBarShadowImage : UINavigationBar.shadowImage，也即导航栏底部那条分隔线，配合 NavBarShadowImageColor 使用。
    QMUICMI.navBarShadowImageColor =UIColorBg243Color ;                              // NavBarShadowImageColor : UINavigationBar.shadowImage 的颜色，如果为 nil，则显示系统默认 shadowImage，如果为全透明，则不显示 shadowImage，如果为除了 nil 和全透明外的其他颜色，则会将这个颜色叠加到 NavBarShadowImage 上显示出来，如果不存在 NavBarShadowImage，则使用一张 1px 高的图片作为默认图。
    QMUICMI.navBarBarTintColor = UIColorNavBarBarTintColor;                                           // NavBarBarTintColor : UINavigationBar.barTintColor，也即背景色
    QMUICMI.navBarStyle = UIBarStyleDefault;                                    // NavBarStyle : UINavigationBar 的 barStyle
    QMUICMI.navBarTintColor = UIColorNavBarTintColor;                                     // NavBarTintColor : QMUINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
    QMUICMI.navBarTitleColor = UIColorNavBarTitleColor;                                 // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
    ...
    QMUICMI.navBarBackIndicatorImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavBack size:CGSizeMake(12, 20) tintColor:UIColorNavBarBackColor];                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片，图片尺寸建议为(13, 21)，否则最终的图片位置无法与系统原生的位置保持一致
    QMUICMI.navBarCloseButtonImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavClose size:CGSizeMake(16, 16) tintColor:UIColorNavBarBackColor];     // NavBarCloseButtonImage : QMUINavigationButton 用到的 × 的按钮图片
    ...
    QMUICMI.navBarAccessoryViewTypeDisclosureIndicatorImage = [[UIImage qmui_imageWithShape:QMUIImageShapeTriangle size:CGSizeMake(8, 5) tintColor:[UIColor whiteColor]] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];     // NavBarAccessoryViewTypeDisclosureIndicatorImage : QMUINavigationTitleView 右边箭头的图片
    
#pragma mark - TabBar
    
    QMUICMI.tabBarBackgroundImage = [[UIImage qmui_imageWithColor:UIColorTabBarBgColor] resizableImageWithCapInsets:UIEdgeInsetsMake(1, 1, 1, 1)];   // TabBarBackgroundImage : UITabBar 的背景图，建议使用 resizableImage，否则在 UITabBar (NavigationController) 的 setBackgroundImage: 里会每次都视为 image 发生了变化（isEqual: 为 NO）
    QMUICMI.tabBarBarTintColor = nil;                                           // TabBarBarTintColor : UITabBar 的 barTintColor
    QMUICMI.tabBarShadowImageColor = UIColorTabBarBgColor;                          // TabBarShadowImageColor : UITabBar 的 shadowImage 的颜色，会自动创建一张 1px 高的图片
    ...
    QMUICMI.tabBarItemTitleColor = UIColorTabBarTitleColor;             // TabBarItemTitleColor : 未选中的 UITabBarItem 的标题颜色
    QMUICMI.tabBarItemTitleColorSelected =UIColorThemeMainColor ;                // TabBarItemTitleColorSelected : 选中的 UITabBarItem 的标题颜色
...
#pragma mark - Toolbar
    ...
    QMUICMI.toolBarTintColor = UIColorThemeMainColor;                            // ToolBarTintColor : UIToolbar 的 tintColor，以及 QMUIToolbarButton normal 状态下的文字颜色
...
//    QMUICMI.searchBarTextFieldBackgroundImage = [UIImage qmui_imageWithColor:UIColorNavBarBarTintColor];       // SearchBarTextFieldBackgroundImage : QMUISearchBar 里的文本框的背景图，图片高度会决定输入框的高度
    QMUICMI.searchBarTextFieldBorderColor = nil;                                // SearchBarTextFieldBorderColor : QMUISearchBar 里的文本框的边框颜色
    QMUICMI.searchBarBackgroundImage = [UIImage qmui_imageWithColor:UIColor.whiteColor];     // SearchBarBackgroundImage : 搜索框的背景图，如果需要设置底部分隔线的颜色也请绘制到图片里
    QMUICMI.searchBarTintColor = UIColorThemeMainColor;                          // SearchBarTintColor : QMUISearchBar 的 tintColor，也即上面的操作控件的主题色
    QMUICMI.searchBarTextColor = UIColor102Color;                                  // SearchBarTextColor : QMUISearchBar 里的文本框的文字颜色
    ...
    QMUICMI.searchBarSearchIconImage = UIImageMake(@"icon_search");                                     // SearchBarSearchIconImage : QMUISearchBar 里的放大镜 icon
...
#pragma mark - TableView / TableViewCell
    ...
    QMUICMI.tableViewBackgroundColor = UIColorViewBgColor;                    // TableViewBackgroundColor : Plain 类型的 QMUITableView 的背景色颜色
    ...
    QMUICMI.tableViewCellTitleLabelColor = UIColor252730Color;             // TableViewCellTitleLabelColor : QMUITableViewCell 的 textLabel 的文字颜色
    QMUICMI.tableViewCellDetailLabelColor = UIColor153Color;     // TableViewCellDetailLabelColor : QMUITableViewCell 的 detailTextLabel 的文字颜色
    QMUICMI.tableViewCellBackgroundColor = UIColorForBackground;                 // TableViewCellBackgroundColor : QMUITableViewCell 的背景色
    QMUICMI.tableViewCellSelectedBackgroundColor = UIColor153Color;  // TableViewCellSelectedBackgroundColor : QMUITableViewCell 点击时的背景色
...
//    QMUICMI.tableViewGroupedCellBackgroundColor = UIColor.qd_backgroundColorLighten;                         // TableViewGroupedCellBackgroundColor : Grouped 类型的 QMUITableView cell 背景色
    ...
    QMUICMI.tableViewGroupedSectionHeaderDefaultHeight = CGFLOAT_MIN;                 // TableViewGroupedSectionHeaderDefaultHeight : Grouped 类型的 QMUITableView sectionHeader 的默认高度（也即没使用自定义的 sectionHeaderView 时的高度），注意如果不需要间距，请用 CGFLOAT_MIN
    QMUICMI.tableViewGroupedSectionFooterDefaultHeight = CGFLOAT_MIN;                 // TableViewGroupedSectionFooterDefaultHeight : Grouped 类型的 QMUITableView sectionFooter 的默认高度（也即没使用自定义的 sectionFooterView 时的高度），注意如果不需要间距，请用 CGFLOAT_MIN
    ...
    QMUICMI.tableViewGroupedSectionFooterContentInset = UIEdgeInsetsMake(0, 0, 0, 0);                 // TableViewGroupedSectionFooterContentInset : Grouped 类型的 QMUITableView sectionFooter 里的内容的 padding
...
- (UIColor *)themeBackgroundColorHighlighted {
    return UIColorNavBarBarTintColor;
}
...
- (UIColor *)themeTitleTextColor {
    return UIColorGray11;
}

- (UIColor *)themeMainTextColor {
    return UIColorGray10;
}

- (UIColor *)themeDescriptionTextColor {
    return UIColorGray12;
}
...
- (UIColor *)themeSeparatorColor {
    return UIColorMake(232, 232, 232);
}
