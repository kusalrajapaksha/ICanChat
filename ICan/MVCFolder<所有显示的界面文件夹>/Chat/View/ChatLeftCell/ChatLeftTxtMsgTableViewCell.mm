...
@interface ChatLeftTxtMsgTableViewCell()<UITextViewDelegate>
...
@end
@implementation ChatLeftTxtMsgTableViewCell
-(void)setcurrentChatModel:(ChatModel *)currentChatModel isShowName:(BOOL)isShowName isGroup:(BOOL)isGroup isShowTime:(BOOL)isShowTime{
    ...
    NSString *text = self.currentChatModel.showMessage;
    UIFont *font = [UIFont systemFontOfSize:16];
    CGSize constrainedSize = CGSizeMake(self.currentChatModel.layoutWidth, CGFLOAT_MAX);
...
-(UIImageView *)containBgImageView{
    if (!_containBgImageView) {
        ...
        _containBgImageView.backgroundColor = [UIColor qmui_colorWithHexString:@"#F6F6F6"];
    }
...
-(UILabel *)timeLabel{
    if (!_timeLabel) {
        _timeLabel = [UILabel leftLabelWithTitle:nil font:10 color:UIColor.darkGrayColor];
    }
    return _timeLabel;
}
...
- (UIView *)replyView{
    if (!_replyView) {
        ...
        _replyView.backgroundColor = [UIColor qmui_colorWithHexString:@"#DDEBFF"];
...
- (UIView *)replyLblContainer{
    if (!_replyLblContainer) {
        ...
        _replyLblContainer.backgroundColor = UIColor.clearColor;
    }
...
- (UIView *)repliedUserLblContainer{
    if (!_repliedUserLblContainer) {
        ...
        _repliedUserLblContainer.backgroundColor = UIColor.clearColor;
    }
...
- (UIView *)reactionView{
    if (!_reactionView) {
        ...
        _reactionView.translatesAutoresizingMaskIntoConstraints = NO;
    }
