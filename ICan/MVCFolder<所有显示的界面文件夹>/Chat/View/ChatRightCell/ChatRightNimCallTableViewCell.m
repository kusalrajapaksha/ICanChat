...
@interface ChatRightNimCallTableViewCell()
...
@end
@implementation ChatRightNimCallTableViewCell
...
-(void)setUpUI{
    ...
    [self.bodyContentView addSubview:self.nimBgView];
...
-(UIImageView *)nimBgImgView{
    if (!_nimBgImgView) {
        _nimBgImgView = [[UIImageView alloc]initWithImage:UIImageMake(@"chat_message_content_mine_bg")];
        //给语音通话的view添加手势
...
-(UILabel *)callLab{
    if (!_callLab) {
        _callLab = [UILabel leftLabelWithTitle:nil font:15 color:UIColor.blackColor];
    }
    return _callLab;
}
