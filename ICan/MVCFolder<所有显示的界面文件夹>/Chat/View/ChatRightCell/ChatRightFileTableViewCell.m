...
-(void)setFileModel:(ChatModel *)fileModel{
    _fileModel = fileModel;
    [self calculateTipsLabel];
}
...
//计算发送的文件进度
-(void)calculateTipsLabel{
    //大于1024*1024 单位是MB
    FileMessageInfo*info=[FileMessageInfo mj_objectWithKeyValues:self.currentChatModel.messageContent];
    if (info) {
        if (self.currentChatModel.uploadState==1) {
            if (self.currentChatModel.totalUnitCount>1024*1024) {
                self.downloadLab.text = [NSString stringWithFormat:@"%.1fMB",info.size/1024.0/1024.0];
            }else{
                self.downloadLab.text=[NSString stringWithFormat:@"%.1fKB",info.size/1024.0];
            }
        }else{
            if (self.currentChatModel.totalUnitCount>1024*1024) {
                self.downloadLab.text=[NSString stringWithFormat:@"%.1fMB/%.1fMB",self.currentChatModel.completedUnitCount/1024.0/1024.0,info.size/1024.0/1024.0];
            }else{
                self.downloadLab.text=[NSString stringWithFormat:@"%.1fKB/%.1fKB",self.currentChatModel.completedUnitCount/1024.0,info.size/1024.0];
            }
        }
    }else{
        if (self.currentChatModel.uploadState==1) {
...
-(void)setUpUI{
    [super setUpUI];
    [self.bodyContentView addSubview:self.fileBgView];
