...
@interface ChatRightGoodsTableViewCell()
...
@end
@implementation ChatRightGoodsTableViewCell
...
-(void)setUpUI{
    ...
    [self.bodyContentView addSubview:self.goodsBgContentView];
...
-(void)clickMessageCell{
    ...
    ChatOtherUrlInfo*goodsInfo=[ChatOtherUrlInfo mj_objectWithKeyValues:self.currentChatModel.messageContent];
    if ([[UIApplication sharedApplication]canOpenURL:[NSURL URLWithString:@"icanmall://"]]) {
        [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"%@ %@",@"Whethertoopen".icanlocalized,goodsInfo.appName] message:nil target:self preferredStyle:UIAlertControllerStyleAlert actionTitles:@[@"UIAlertController.cancel.title".icanlocalized,@"UIAlertController.sure.title".icanlocalized] handler:^(int index) {
            if (index==1) {
                NSString*share = [NSString stringWithFormat:@"icanmall://iCanShare?extra=%@",goodsInfo.extra];
                NSURL*url = [NSURL URLWithString:share.netUrlEncoded];
