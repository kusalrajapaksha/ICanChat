...
@interface ChatListViewController : QDCommonTableViewController
...
/*
 ...
 直接调用咱们封装好的ChatHandler的sendApplyAddFriendMessage方法即可 ， 发送消息时 ，需要存入或者更新chatlist和chatinfo两张表。若是未连接或者发送超时 ，需要重新更新数据库存储的发送成功与否状态 ，同时更新内存数据源 ，刷新该条消息展示即可。
