...
// https://faq.yunxin.163.com/kb/main/#/heading/20/通信类/推送
...
//    NSString *appName = NSBundle.mainBundle.infoDictionary[@"CFBundleDisplayName"];
    ...
    if ([self.authorityType isEqualToString:AuthorityType_circle]) {
        NIMSignalingChannelType channelType = self.context.channelInfo.channelType;
        NSString *typeString = channelType == NIMSignalingChannelTypeVideo ? @"InviteYouVideoCall".icanlocalized : @"InviteYouVoiceCall".icanlocalized;
        info.pushContent = [NSString stringWithFormat:@"%@",typeString];
        info.pushTitle = [CircleUserInfoManager shared].nickname;
        info.pushPayload = @{@"apsField": @{@"sound": @"NIMPushCall.wav",@"alert": @{@"title":[CircleUserInfoManager shared].nickname,@"body": [NSString stringWithFormat:@"%@",typeString]}}};
    }else{
        NIMSignalingChannelType channelType = self.context.channelInfo.channelType;
        NSString *typeString = channelType == NIMSignalingChannelTypeVideo ? @"InviteYouVideoCall".icanlocalized : @"InviteYouVoiceCall".icanlocalized;
        NSMutableDictionary *myPayloadDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myApsDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myAlertDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myFcmDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myMessageDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myDataDictionary = [NSMutableDictionary dictionary];
        NSMutableDictionary *myAndroidDictionary = [NSMutableDictionary dictionary];
        [myApsDictionary setObject:@"NIMPushCall.wav" forKey:@"sound"];
        [myAlertDictionary setObject:[UserInfoManager sharedManager].nickname forKey:@"title"];
        [myAlertDictionary setObject:[NSString stringWithFormat:@"%@",typeString] forKey:@"body"];
        [myApsDictionary setObject: myAlertDictionary forKey:@"alert"];
        [myPayloadDictionary setObject: myApsDictionary forKey:@"apsField"];
        [myFcmDictionary setObject: @"false" forKey:@"validate_only"];
        [myFcmDictionary setObject: myMessageDictionary forKey:@"message"];
        [myMessageDictionary setObject: @"false" forKey:@"needNotification"];
        [myMessageDictionary setObject: myDataDictionary forKey:@"data"];
        [myDataDictionary setObject: [UserInfoManager sharedManager].nickname forKey:@"pushTitle"];
        [myDataDictionary setObject: [UserInfoManager sharedManager].userId forKey:@"fromAccountId"];
        [myDataDictionary setObject: @"true" forKey:@"pushCallNotification"];
        [myDataDictionary setObject: [NSString stringWithFormat:@"%@",typeString] forKey:@"pushContent"];
        if(channelType == NIMSignalingChannelTypeVideo){
            [myDataDictionary setObject: @"VIDEO" forKey:@"channelType"];
        }else {
            [myDataDictionary setObject: @"AUDIO" forKey:@"channelType"];
        }
        [myDataDictionary setObject: invite.requestId forKey:@"requestedId"];
        [myDataDictionary setObject: self.context.channelInfo.channelId forKey:@"channelId"];
        [myMessageDictionary setObject: myAndroidDictionary forKey:@"android"];
        [myAndroidDictionary setObject: @{} forKey:@"fcm_options"];
        [myAndroidDictionary setObject: myDataDictionary forKey:@"data"];
        [myAndroidDictionary setObject: @(YES) forKey:@"direct_boot_ok"];
        [myPayloadDictionary setObject: myFcmDictionary forKey:@"fcmFieldV1"];
        [myPayloadDictionary setObject: callerUserID forKey: @"callerID"];
        info.pushContent = [NSString stringWithFormat:@"%@",typeString];
        info.pushTitle = [UserInfoManager sharedManager].nickname;
        info.pushPayload = myPayloadDictionary;
    }
    if (callerUserID) {
...
- (void)joinRtcChannel:(NSString *)channelID
    ...
    if (!token) {
        NSLog(@"CK: Cannot Join RTC with a nil token!!");
        return;
    ...
    int ret = [NERtcEngine.sharedEngine joinChannelWithToken:token channelName:channelID myUid:myUid completion:^(NSError * _Nullable error, uint64_t channelId, uint64_t elapesd, uint64_t uid) {
