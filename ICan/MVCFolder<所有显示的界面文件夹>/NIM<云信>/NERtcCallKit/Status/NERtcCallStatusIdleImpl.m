...
- (void)call:(NSString *)userID type:(NERtcCallType)type completion:(void (^)(NSError * _Nullable))completion {
    ...
    if ([[NERtcCallKit sharedInstance].authorityType isEqualToString:AuthorityType_circle]) {
        NSDictionary*dict=@{@"authorityType":[NERtcCallKit sharedInstance].authorityType,@"circleUserId":CircleUserInfoManager.shared.userId};
        create.channelExt=[dict mj_JSONString];
    }
    // 1. 创建信令频道
