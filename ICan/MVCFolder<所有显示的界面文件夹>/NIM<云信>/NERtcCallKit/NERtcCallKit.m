...
- (void)setupAppKey:(NSString *)appKey
    ...
    option.pkCername = @"Certificatesvoip";
    ...
    config.degradationPreference=kNERtcDegradationMaintainQuality;
    ...
    [[NIMSDK sharedSDK].apnsManager updateApnsSetting:setting completion:nil];
...
- (void)setupRemoteView:(UIView *)remoteView forUser:(nonnull NSString *)userID {
    ...
    if (!member) {
        return;
        NSLog(@"Error: member of userID: %@ does NOT exist", userID);
    }
    if (remoteView) {
        NERtcVideoCanvas *canvas = [[NERtcVideoCanvas alloc] init];
        canvas.renderMode = kNERtcVideoRenderScaleCropFill;
...
//            [self performSelector:@selector(onTimeout) withObject:self afterDelay:5]; --> This line need to resolve "Other party left issue" ...!
            [self cancelTimeout];
...
- (void)_handleInviteInfo:(NIMSignalingInviteNotifyInfo *)info {
    ...
    NSDictionary*dict=[info.channelInfo.channelExt mj_JSONObject];
    if (dict) {
        self.circleUserId=[dict objectForKey:@"circleUserId"];
        self.authorityType=[dict objectForKey:@"authorityType"];;
    }
