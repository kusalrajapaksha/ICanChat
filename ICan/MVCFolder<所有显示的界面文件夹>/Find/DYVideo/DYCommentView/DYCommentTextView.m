...
static CGFloat const KFaceViewHeight=200;
@interface DYCommentTextView ()<QMUITextViewDelegate,XMChatFaceViewDelegate>
...
-(void)setUpView{
    ...
    [self addSubview:self.commentfaceView];
}
...
-(void)hiddenFaceView{
    ...
    self.mineHeight=self.textViewHeight+10+self.bottomMargin;
    self.commentfaceView.frame=CGRectMake(0, KHeightRatio(440), ScreenWidth, KFaceViewHeight);
...
-(XMChatFaceView *)commentfaceView{
    if (!_commentfaceView) {
        _commentfaceView = [[XMChatFaceView alloc] initWithFrame:(CGRectMake(0, KWidthRatio(440), ScreenWidth, KFaceViewHeight))];
...
//        _textView.returnKeyType = UIReturnKeySend;
        _textView.maximumHeight=MaxTextViewHeight;
...
-(void)updateConstraintsFram{
    ...
    if (self.isComment) {
        self.frame=CGRectMake(0, KHeightRatio(440)-self.mineHeight, ScreenWidth, self.mineHeight);
        ...
        !self.frameChangeFrameBlock?:self.frameChangeFrameBlock(self.mineHeight);
        self.topBgView.hidden=YES;
    }else{
        //30是那个回复谁的提示
        self.topBgView.hidden=NO;
//        KHeightRatio(440)-self.mineHeight-30
        self.frame=CGRectMake(0,KHeightRatio(440)-self.mineHeight , ScreenWidth, self.mineHeight+30);
        [self.bgKeyboardView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(self.textViewHeight));
            make.top.equalTo(@(40));
            make.right.equalTo(@-55);
