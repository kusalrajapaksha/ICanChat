...
//    }
    ...
    sheetViewF.size.height = BtnHeight * _tag + Margin+bottomMargin;
    sheetView.frame = sheetViewF;
    // 取消按钮
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, sheetView.frame.size.height - BtnHeight-bottomMargin, ScreenWidth, BtnHeight)];
    btn.contentEdgeInsets = UIEdgeInsetsMake(5, 5, 5, 5);
    [btn setBackgroundImage:normalImage forState:UIControlStateNormal];
    [btn setBackgroundImage:highImage forState:UIControlStateHighlighted];
    [btn setTitle:cancelTitle forState:UIControlStateNormal];
    [btn setTitleColor:UIColor252730Color forState:UIControlStateNormal];
...
- (void)setBtnTag:(NSInteger)tag TextColor:(UIColor *)color textFont:(CGFloat)font enable:(BOOL)enable {
    ...
    UIButton *btn = [self.sheetView viewWithTag:tag];
    if (color) {
