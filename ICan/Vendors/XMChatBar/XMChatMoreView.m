...
@property (copy, nonatomic) NSArray *itemIndex;
...
- (void)reloadData{
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(titlesOfMoreView:)]) {
        self.titles = [self.dataSource titlesOfMoreView:self];
    }
    
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(imageNamesOfMoreView:)]) {
        self.imageNames = [self.dataSource imageNamesOfMoreView:self];
    }
    
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(itemIndexOfMoreView:)]) {
        self.itemIndex = [self.dataSource itemIndexOfMoreView:self];
    }
    
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(itemsOfMoreView:)]) {
        self.items = [self.dataSource itemsOfMoreView:self];
    }
...
- (void)setupItems{
    ...
    if (self.items && self.items.count != 0) {
        ...
        [self.items enumerateObjectsUsingBlock:^(ChatMoreTagItem *tagItem, NSUInteger idx, BOOL *stop) {
            ...
            [item fillViewWithTitle:tagItem.title imageName:tagItem.imageStr];
            // 设置点击的 下角标
            item.tag = tagItem.tag;
        ...
        [self.titles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL *stop) {
            ...
            // 设置点击的 下角标
            item.tag = [self.itemIndex[idx] integerValue];
            [item addTarget:self action:@selector(itemClickAction:) forControlEvents:UIControlEventTouchUpInside];
            [self.scrollView addSubview:item];
            [self.itemViews addObject:item];
            column ++ ;
