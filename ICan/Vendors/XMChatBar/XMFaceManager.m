...
@property (strong, nonatomic) NSMutableArray <NSTextCheckingResult*> * textCheckingResultMatchArrays;
@end
...
+ (NSMutableAttributedString *)getEmotionStrWithString:(NSString *)text fontSize:(CGFloat)fontSize color:(UIColor*)color{
   //1、创建一个可变的属性字符串
    ...
    //    @try {
    ////        [attributeString addAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14.0f]} range:NSRangeFromString(kNotNIL(text))];
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (NSDictionary *dict in [[XMFaceManager shareInstance] emojiFaceArrays]) {
            if ([dict[kFaceNameKey]  isEqualToString:subStr]) {
                ...
                textAttachment.bounds = CGRectMake(0, -8, textAttachment.image.size.width-3 , textAttachment.image.size.height-3);
...
+ (NSMutableAttributedString *)emotionStrWithString:(NSString *)text{
    ...
    NSDictionary*dic = @{
        NSFontAttributeName: font,
        NSParagraphStyleAttributeName: style,
        NSForegroundColorAttributeName:UIColor252730Color
    };
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (NSDictionary *dict in [[XMFaceManager shareInstance] emojiFaceArrays]) {
            if ([dict[kFaceNameKey]  isEqualToString:subStr]) {
                ...
                textAttachment.image = [UIImage imageNamed:dict[kFaceImageNameKey]];
                //调整一下图片的位置,如果你的图片偏上或者偏下，调整一下bounds的y值即可
                textAttachment.bounds = CGRectMake(0, -8, textAttachment.image.size.width-7 , textAttachment.image.size.width-7);
                //把附件转换成可变字符串，用于替换掉源字符串中的表情文字
                NSAttributedString *imageStr = [NSAttributedString attributedStringWithAttachment:textAttachment];
                //把图片和图片对应的位置存入字典中
                NSMutableDictionary *imageDic = [NSMutableDictionary dictionaryWithCapacity:2];
...
+ (NSMutableAttributedString *)getReplyEmotionStrWithString:(NSString *)text isShowReplayView:(BOOL)isShowReplayView{
    ...
    NSDictionary*dic = @{
        ...
        NSParagraphStyleAttributeName: style,
        NSKernAttributeName:@(0.2),
        NSForegroundColorAttributeName:isShowReplayView?UIColor252730Color:UIColor153Color
    };
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (NSDictionary *dict in [[XMFaceManager shareInstance] emojiFaceArrays]) {
            if ([dict[kFaceNameKey]  isEqualToString:subStr]) {
                ...
                textAttachment.image = [UIImage imageNamed:dict[kFaceImageNameKey]];
                //调整一下图片的位置,如果你的图片偏上或者偏下，调整一下bounds的y值即可
                textAttachment.bounds = CGRectMake(0, -8, textAttachment.image.size.width-6 , textAttachment.image.size.height-6);
                //把附件转换成可变字符串，用于替换掉源字符串中的表情文字
                NSAttributedString *imageStr = [NSAttributedString attributedStringWithAttachment:textAttachment];
                //把图片和图片对应的位置存入字典中
                NSMutableDictionary *imageDic = [NSMutableDictionary dictionaryWithCapacity:2];
...
+ (NSMutableAttributedString *)emotionStrWithString:(NSString *)text isOutGoing:(BOOL)isOutGoing{
    ...
    NSDictionary*dic = @{
        ...
        NSParagraphStyleAttributeName: style,
        NSKernAttributeName:@(0.2),
        NSForegroundColorAttributeName:isOutGoing? UIColorChatRightTextColor:UIColorChatLeftTextColor
    };
